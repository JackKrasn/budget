...
export function CancellationResultDialog({
  ...
  // Get first balance changes (assuming single asset for now)
  const accountBalance = result.accountBalances?.[0]
  const fundBalance = result.fundBalances?.[0]
            ...
            <Card className="border-emerald-500/30 bg-emerald-500/5">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                  +{formatMoney(Math.abs(accountBalance.change))} ₽
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {fundBalance && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-sm font-medium">Фонд ({fundBalance.assetName})</h4>
              </div>
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="mt-2 pt-2 border-t border-border/50">
                    <p className="text-xs text-muted-foreground">
                      ...
                      <span className="font-medium text-rose-600 dark:text-rose-400">
                        {formatMoney(fundBalance.change)} ₽
                      </span>
                    </p>
                  </div>
                </CardContent>
