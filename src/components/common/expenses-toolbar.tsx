...
export type ExpenseViewMode = 'categories' | 'tags' | 'list'

export interface ExpensesToolbarProps {
  // View mode
      ...
      >
        ...
        {isActive ? (
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-medium truncate max-w-[100px]">{selectedLabel}</span>
          </div>
        ) : (
          <span>{label}</span>
        )}
    ...
    <Card className="border-border/40 bg-card/30 backdrop-blur-sm">
      <div className="flex flex-wrap items-center gap-3 p-3">
        ...
        <ToggleGroup
          ...
          value={viewMode}
          onValueChange={(value) => {
            if (value) onViewModeChange(value as ExpenseViewMode)
          }}
        ...
        >
          <ToggleGroupItem
            ...
            className="h-8 px-3 gap-1.5 data-[state=on]:bg-background data-[state=on]:shadow-sm rounded-md"
          >
              ...
              trigger={
                <div>
                  <FilterButton
                    label="Счёт"
                    icon={Wallet}
                    selectedLabel={selectedAccount ? `${selectedAccount.name}` : undefined}
